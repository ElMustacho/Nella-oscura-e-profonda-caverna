...
TEST(TestFactory, TestObjectFactoryUniquePointers) {
	std::cout << "Test sulla creazione di oggetti con Object Factory.\n";
	// Da qui fino al prossimo commento è solo fase di preparazione
	Oggetto oggetto1(10, "Qualcosa di unico", "Ciò che non è ben specificato", 20), oggetto2(0.2, "Forbici uniche", "È sconsigliata la corsa", 15), oggetto3(9000, "9 metri unici cubi d'acqua", "In caso di disidratazione, consumarne una piccola frazione", 500);
	Arma arma1(3, "L'unico tagliaunghie atomico", "Contro le unghie radioattive non c'è arma migliore.", 4, Danno({ 1 }, 4)), arma2(2.1, "Spada Lasel unica", "Spada cleata pel i guellieli Jedi con la elle moscia", 150, Danno({ 0.5,0,0,0.5 }, 35));
	std::vector<std::shared_ptr<Oggetto>> soloObj;
	std::vector<std::shared_ptr<Arma>> soloWeap;
	std::vector<std::shared_ptr<Oggetto>> mixed;
	soloObj.push_back(std::make_shared<Oggetto>(oggetto1));
